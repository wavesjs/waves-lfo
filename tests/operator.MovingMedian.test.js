import MovingMedian from '../src/common/operator/MovingMedian';
...
tape('MovingMedian', (t) => {
  ...
  sMedian.processStreamParams({ frameSize: 1 });
  sMedian.resetStream();
  ...
  aMedian.processStreamParams({ frameSize: 2 });
  aMedian.resetStream();
  ...
  for (let i = 0; i < values.length; i++) {
    const median = aMedian.inputVector(values[i]);
  ...
  fMedian.processStreamParams({ frameSize: 2 });
  fMedian.resetStream();
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    const result = fMedian.inputVector([sample0, sample1]);
